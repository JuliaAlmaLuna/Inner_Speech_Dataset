...
def Transform_for_classificator(X, Y, Classes, Conditions):
    ...
    for N_gr in range(N_grups_cnd):
        ...
        for N_ind in range(N_ind_clas):
            ...
            if N_ind == 0 and N_gr == 0:
                ...
                # print(Y_finalAux.shape)
                # print(Y_aux.shape)
                # print("dfa")
                # test = np.concatenate([Y_finalAux, Y_aux], axis=1)
                # Y_finalAux = np.concatenate(
                #     [Y_finalAux, Y_aux], axis=0
                # )  # np.hstack([Y_finalAux, Y_aux])

                Y_finalAux = np.hstack([Y_finalAux, Y_aux])
                # Y_finalAux2 = np.hstack([Y_finalAux, Y_aux])
                Y_finalAux2 = np.concatenate(
                    [Y_finalAux2, Y_aux], axis=0
                )
    ...
    # print(Y_finalAux2)
    # print("guuuo")
    # print(np.reshape(Y_finalAux, [-1, 4]))
    # print("julia")
    # print(np.reshape(Y_finalAux2, [-1, 4]))
    # time.sleep(1000)
    return X_final, Y_final, Y_finalAux
...
def Filter_by_class(X, Y, class_condition):
    ...
    if class_condition.upper() == "ALL":
        ...
        if class_condition.upper() == "UP" or class_condition.upper() == "ARRIBA":
            ...
            class_condition.upper() == "LEFT" or class_condition.upper() == "IZQUIERDA"
        ):
            p = 3
        else:
